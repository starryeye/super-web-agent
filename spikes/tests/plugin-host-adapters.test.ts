...
function recorder(records: string[]): RunCommand {
  return async ({ displayName, args, env }) => {
    ...
    if (env !== undefined) {
      expect(Object.keys(env).sort()).toEqual(["SWA_SPIKE_EVIDENCE_PATH", "SWA_SPIKE_HOST", "SWA_SPIKE_PLUGIN_VERSION", "SWA_SPIKE_RUN_ID", "PARENT"].sort());
    }
...
it.each([
  ["claude-code", "claude", "/private/tmp/super-web-agent-lifecycle-fixture/active/claude-code", [
    "claude --version",
    "claude plugin marketplace add /private/tmp/super-web-agent-lifecycle-fixture/active/claude-code",
    "claude plugin install super-web-agent-lifecycle-spike@super-web-agent-lifecycle-spike-claude --scope user",
    `claude -p ${healthPrompt("adapter-test")} --output-format json --permission-mode dontAsk`,
    "claude plugin marketplace update super-web-agent-lifecycle-spike-claude",
    "claude plugin update super-web-agent-lifecycle-spike@super-web-agent-lifecycle-spike-claude --scope user",
    "claude plugin uninstall super-web-agent-lifecycle-spike@super-web-agent-lifecycle-spike-claude --scope user",
    "claude plugin marketplace remove super-web-agent-lifecycle-spike-claude",
  ]],
  ["codex", "codex", "/private/tmp/super-web-agent-lifecycle-fixture/active/codex", [
    "codex --version",
    "codex plugin marketplace add /private/tmp/super-web-agent-lifecycle-fixture/active/codex --json",
    "codex plugin add super-web-agent-lifecycle-spike@super-web-agent-lifecycle-spike-codex --json",
    `codex exec --ephemeral --sandbox read-only -c approval_policy="never" --json ${healthPrompt("adapter-test")}`,
    "codex plugin add super-web-agent-lifecycle-spike@super-web-agent-lifecycle-spike-codex --json",
    "codex plugin remove super-web-agent-lifecycle-spike@super-web-agent-lifecycle-spike-codex --json",
    "codex plugin marketplace remove super-web-agent-lifecycle-spike-codex --json",
  ]],
  ...
  await adapter.runPrompt(healthPrompt("adapter-test"), "/project", { PARENT: "allowed", SWA_SPIKE_EVIDENCE_PATH: "/evidence", SWA_SPIKE_HOST: host, SWA_SPIKE_RUN_ID: "run", SWA_SPIKE_PLUGIN_VERSION: "0.0.1" });
...
it("resolves package bin strings and keyed bins inside its package root", async () => {
  const prefix = await mkdtemp(join(tmpdir(), "super-web-agent-bin-")); directories.push(prefix);
...
it("preserves Claude's successful marketplace update when the plugin update runner rejects", async () => {
  ...
  await expect(adapter.update("/project")).rejects.toMatchObject({ partialObservations: [{ command: "claude plugin marketplace update super-web-agent-lifecycle-spike-claude" }] });
});
